...
const IbanValidator = Jymfony.Component.Validator.Constraints.IbanValidator;
const { expect } = require('chai');

describe('[Validator] Constraints.IbanValidator', function () {
    it ('null should be valid', async () => {
        await expect(null).to.be.validated.by(IbanValidator)
            .with.constraint(new Iban())
            .and.raise.no.violations();
    });

    it ('undefined should be valid', async () => {
        await expect(undefined).to.be.validated.by(IbanValidator)
            .with.constraint(new Iban())
            .and.raise.no.violations();
    });

    it ('empty string should be valid', async () => {
        await expect('').to.be.validated.by(IbanValidator)
            .with.constraint(new Iban())
            .and.raise.no.violations();
    });
    ...
    for (const iban of validIbans) {
        it('should validate iban #' + ++i, async () => {
            await expect(iban).to.be.validated.by(IbanValidator)
                .with.constraint(new Iban())
                .and.raise.no.violations();
        });
    }

    const ibansWithInvalidFormats = [
    ...
    i = 0;
    for (const iban of ibansWithInvalidFormats) {
        it('invalid iban format should raise violation #' + ++i, async () => {
            await expect(iban).to.be.validated.by(IbanValidator)
                .with.constraint(new Iban({ message: 'myMessage' }))
                .and.raise.violations([ {
                    message: 'myMessage',
                    code: Iban.INVALID_FORMAT_ERROR,
                    parameters: {
                        '{{ value }}': '"' + iban + '"',
                    },
                    propertyPath: '',
    ...
    const ibansWithValidFomatButIncorrectChecksums = [
    ...
    i = 0;
    for (const iban of ibansWithValidFomatButIncorrectChecksums) {
        it('invalid iban checksum should raise violation #' + ++i, async () => {
            await expect(iban).to.be.validated.by(IbanValidator)
                .with.constraint(new Iban({ message: 'myMessage' }))
                .and.raise.violations([ {
                    message: 'myMessage',
                    code: Iban.CHECKSUM_FAILED_ERROR,
                    parameters: {
                        '{{ value }}': '"' + iban + '"',
                    },
                    propertyPath: '',
    ...
    i = 0;
    for (const cc of unsupportedCountryCodes) {
        it('unsupported country codes should raise violation #' + ++i, async () => {
            await expect(cc + '260211000000230064016').to.be.validated.by(IbanValidator)
                .with.constraint(new Iban({ message: 'myMessage' }))
                .and.raise.violations([ {
                    message: 'myMessage',
                    code: Iban.NOT_SUPPORTED_COUNTRY_CODE_ERROR,
                    parameters: {
                        '{{ value }}': '"' + cc + '260211000000230064016' + '"',
                    },
                    propertyPath: '',
                } ]);
        });
    ...
    it('invalid characters should raise violation', async () => {
        await expect('CH930076201162385295]').to.be.validated.by(IbanValidator)
            .with.constraint(new Iban({ message: 'myMessage' }))
            .and.raise.violations([ {
                message: 'myMessage',
                code: Iban.INVALID_CHARACTERS_ERROR,
                parameters: {
                    '{{ value }}': '"CH930076201162385295]"',
                },
                propertyPath: '',
            } ]);
    });
    ...
    i = 0;
    for (const iban of invalidCountryCodes) {
        it('invalid country codes should raise violation #' + ++i, async () => {
            await expect(iban).to.be.validated.by(IbanValidator)
                .with.constraint(new Iban({ message: 'myMessage' }))
                .and.raise.violations([ {
                    message: 'myMessage',
                    code: Iban.INVALID_COUNTRY_CODE_ERROR,
                    parameters: {
                        '{{ value }}': '"' + iban + '"',
                    },
                    propertyPath: '',
