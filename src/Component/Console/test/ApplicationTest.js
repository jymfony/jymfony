...
const ensureStaticCommandHelp = (application) => {
    for (const command of Object.values(application.all())) {
...
describe('[Console] Application', function () {
    ...
    it('has get should work', () => {
        ...
        const foo = new Fixtures.FooCommand();
        application.add(foo);
    ...
    const ambiguousAbbreviations = function * () {
    ...
    for (const [ abbrev, message ] of ambiguousAbbreviations()) {
    ...
    const invalidCommandNamesSingle = function * () {
        yield 'foo3:baR';
        yield 'foO3:bar';
    };

    for (const name of invalidCommandNamesSingle()) {
    ...
    it('find should return correct command with alias', () => {
        const command = new Fixtures.FooCommand();
