...
const YamlFileLoader = Jymfony.Component.Routing.Loader.YamlFileLoader;
...
describe('[Routing] YamlFileLoader', function () {
    it ('should support yaml files', () => {
        const loader = new YamlFileLoader(null);
        ...
        expect(loader.supports('foo.yaml', 'yaml'), 'supports() checks the resource type if specified').to.be.true;
        expect(loader.supports('foo.yaml', 'foo'), 'supports() checks the resource type if specified').to.be.false;
    });

    it ('should do nothing if file is empty', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir ]));
        const collection = loader.load('empty.yaml');

        expect(collection.all()).to.be.deep.eq({});
        expect(collection.resources).to.be.deep.eq([ new FileResource(realpathSync(fixturesDir + '/empty.yaml')) ]);
    });

    const invalidFiles = [
        'nonvalid.yaml', 'nonvalid2.yaml',
        'incomplete.yaml', 'nonvalidkeys.yaml',
        'nonesense_resource_plus_path.yaml',
        'nonesense_type_without_resource.yaml',
    ];
    ...
    it ('should load special route name', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir ]));
        const routeCollection = loader.load('special_route_name.yaml');
    ...
    it ('should load routes', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir ]));
        const routeCollection = loader.load('validpattern.yaml');
    ...
    it ('should load resources', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir ]));
        const routeCollection = loader.load('validresource.yaml');
    ...
    it ('should load route with controller attribute', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/controller' ]));
        const routeCollection = loader.load('routing.yaml');
    ...
    it ('should throw with controller set in defaults and controller attribute', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/controller' ]));
        expect(() => loader.load('override_defaults.yaml')).to.throw(
    ...
    const importFile = [
        'import_controller.yaml',
        'import__controller.yaml',
    ];
    ...
    it ('should throw on import routes with overridden controller', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/controller' ]));
        expect(() => loader.load('import_override_defaults.yaml')).to.throw(
            InvalidArgumentException,
    ...
    it ('should import routes with glob matching single file', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/glob' ]));
        const routeCollection = loader.load('import_single.yaml');
    ...
    it ('should import routes with glob matching multiple file', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/glob' ]));
        const routeCollection = loader.load('import_multiple.yaml');
    ...
    it ('should import routes with name prefix', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/import_with_name_prefix' ]));
        const routeCollection = loader.load('routing.yaml');
    ...
    it ('should load route with defaults', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir ]));
        const routes = loader.load('defaults.yaml');
    ...
    it ('should import routes with defaults', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir ]));
        const routes = loader.load('importer-with-defaults.yaml');
        ...
        expectedRoutes.addResource(new FileResource(fixturesDir + '/imported-with-defaults.yaml'));
        expectedRoutes.addResource(new FileResource(fixturesDir + '/importer-with-defaults.yaml'));

        expect(routes).to.be.deep.eq(expectedRoutes);
    });

    it ('should load localized routes', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        const routes = loader.load('localized-route.yaml');
    ...
    it ('should load imported localized routes', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        const routes = loader.load('importing-localized-route.yaml');
    ...
    it ('should load import routes with locale', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        const routes = loader.load('importer-with-locale.yaml');
    ...
    it ('should load import non-localized routes with locale', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        const routes = loader.load('importer-with-locale-imports-non-localized-route.yaml');
    ...
    it ('should load import routes with official locale', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        const routes = loader.load('officially_formatted_locales.yaml');
    ...
    it ('should throw importing routes with missing locale prefix', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        expect(() => loader.load('missing-locale-in-importer.yaml')).to.throw(InvalidArgumentException);
    });

    it ('should throw importing routes without path or locales', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        expect(() => loader.load('route-without-path-or-locales.yaml')).to.throw(InvalidArgumentException);
    });

    it ('should import with controller default', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/localized' ]));
        const routes = loader.load('importer-with-controller-default.yaml');
        expect(routes).to.have.length(3);
    ...
    it ('should import routes with no trailing slash', () => {
        const loader = new YamlFileLoader(new FileLocator([ fixturesDir + '/import_with_no_trailing_slash' ]));
        const routeCollection = loader.load('routing.yaml');
