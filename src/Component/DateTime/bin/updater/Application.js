...
export default class Application extends SingleCommandApplication {
    ...
    async execute(input, output) {
        ...
        const eslintrc = new OpenFile(baseDir + '/.eslintrc.json', 'w');
...
`));
        ...
        for (const zone of Object.values(this._parser.zones)) {
            ...
            const zoneFile = new OpenFile(baseDir + '/timezones/' + name + '.js', 'w');
        ...
        for (const [ name, main ] of __jymfony.getEntries(this._parser.aliases)) {
            ...
            const zoneFile = new OpenFile(aliasFn, 'w');
        ...
        for (const [ name, details ] of __jymfony.getEntries(this._parser.abbrevs)) {
            ...
            const zoneFile = new OpenFile(baseDir + '/abbrev/' + name + '.js', 'w');
        ...
        const abbrevsJson = new OpenFile(baseDir + '/abbrevs.json', 'w');
        ...
        const zonesJson = new OpenFile(baseDir + '/zones.json', 'w');
