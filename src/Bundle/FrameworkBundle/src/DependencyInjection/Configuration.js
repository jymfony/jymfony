        ...
        rootNode
            .children()
                .arrayNode('messenger')
                    ...
                    .validate()
                        .ifTrue(v => !! v.buses && 1 < Object.keys(v.buses).length && null === v.default_bus)
                        .thenInvalid('You must specify the "default_bus" if you define more than one bus.')
                    .end()
                    .validate()
                        .ifTrue(v => !! v.buses && null !== v.default_bus && !v.buses[v.default_bus])
                        .then(v => {
                            throw new InvalidConfigurationException(__jymfony.sprintf('The specified default bus "%s" is not configured. Available buses are "%s".', v.default_bus, Object.keys(v.buses).join('", "')));
                        })
                    .end()
                    .children()
                        .arrayNode('routing')
